...
const APP_AUTH_MODES = new Set(["desktop", "android"]);
...
function getAppMode(req) {
  return String(req.headers["x-threadborn-app-mode"] || "").trim().toLowerCase();
}

function isWrappedAppOrigin(req) {
  const originHost = getOriginHost(req);
  return originHost === "appassets.androidplatform.net" || originHost.startsWith("127.0.0.1") || originHost.startsWith("localhost");
}

function shouldExposeSessionToken(req) {
  return APP_AUTH_MODES.has(getAppMode(req)) || isWrappedAppOrigin(req);
}
...
function needsCrossSiteSessionCookie(req) {
...
function makeCookie(name, value, maxAge, options = {}) {
  ...
  const secure = options.secure === undefined ? process.env.NODE_ENV === "production" : Boolean(options.secure);
  const parts = [`${name}=${encodeURIComponent(value)}`, "Path=/", "HttpOnly", `SameSite=${sameSite}`, `Max-Age=${maxAge}`];
  if (secure) {
    parts.push("Secure");
  }
  if (options.clear) {
...
async function destroySession(req, res) {
  ...
  const tokens = [cookies[SESSION_COOKIE], getBearerToken(req)].filter(Boolean);
  if (tokens.length) {
    const hashes = tokens.map(hashToken);
    await pool.query("delete from sessions where token_hash = any($1::text[])", [hashes]);
  }
  res.setHeader("Set-Cookie", makeCookie(SESSION_COOKIE, "", 0, getSessionCookieOptions(req, { clear: true })));
}

async function getSession(req) {
  await pool.ensureMigrations();
  const cookies = parseCookies(req);
  const sessionToken = cookies[SESSION_COOKIE] || getBearerToken(req);
  if (!sessionToken) {
    return null;
  }
...
function getSessionCookieOptions(req, overrides = {}) {
  const crossSite = needsCrossSiteSessionCookie(req);
